...
def define_period(df, time):
    ...
    if time == 'year':
        ...
        l_val = int(l_train + 0.3 * len(df))  # da 31536 a 42048, cioè 10512 valori (per un anno)
        df_def = df
...
class MLP(nn.Module):
    ...
    def __init__(self, n_features):
        ...
        self.hidden1 = Linear(n_features, 240) # input to first hidden layer
        self.act1 = ReLU()
        self.hidden2 = Linear(240, 100)
        self.act2 = ReLU()
...
def train_model(model, epochs, train_dl, val_dl, optimizer, criterion, lr_scheduler, mode=''):
    ...
    for t in range(epochs):
        ...
        for x, label in train_dl:
            ...
            loss_c = criterion(output.view(-1), label.float())
            optimizer.zero_grad()
            loss_c.backward()
            optimizer.step()
        ...
        for inputs, labels in val_dl:
            ...
            val_loss_c = criterion(val_output.view(-1), labels.float())
